...
class BuildingOperation(CrudOperation):
    ...
    async def get_buildings(self, page: int = 1, page_size: int = 10):
        # async with self.db_session as session:
            ...
            total_query = await self.db_session.execute(select(func.count(DBBuilding.id)))
    ...
    async def update_building(self, building_id: int, building):
        # async with self.db_session as session:
            ...
            try:
                db_building = await self.db_session.merge(db_building)
    ...
    async def delete_building(self, building_id: int):
        # async with self.db_session as session:
            ...
            try:
                db_building = await self.db_session.merge(db_building)
                await self.db_session.delete(db_building)
                await self.db_session.commit()
                return db_building
            except SQLAlchemyError as e:
                await self.db_session.rollback()
...
class GateOperation(CrudOperation):
    ...
    async def get_gate(self, gate_id: int):
        # async with self.db_session as session:
            ...
            query = await self.db_session.execute(select(DBGate)
    ...
    async def get_gates(self, page: int = 1, page_size: int = 10):
        # async with self.db_session as session:
            # Calculate total number of records
            total_query = await self.db_session.execute(select(func.count(DBGate.id)))
            total_records = total_query.scalar_one()
    ...
    async def create_gate(self, gate):
        # async with self.db_session as session:
            db_building = await BuildingOperation(self.db_session).get_building(gate.building_id)
            try:
                db_building = await self.db_session.merge(db_building)
        ...
        # async with self.db_session as session:
            ...
            try:
                ...
                if "building_id" in update_data:
                    ...
                    await BuildingOperation(self.db_session).get_building(building_id)
    ...
    async def delete_gate(self, gate_id: int):
        # async with self.db_session as session:
            ...
            try:
                ...
                await self.db_session.delete(db_gate)
                await self.db_session.commit()
                return db_gate
            except SQLAlchemyError as e:
                await self.db_session.rollback()
...
class SettingOperation(CrudOperation):
    ...
    async def get_setting_by_name(self, name: str):
        # async with self.db_session as session:
            query = await self.db_session.execute(select(DBCameraSetting).where(DBCameraSetting.name == name))
            setting = query.unique().scalar_one_or_none()
            return setting
    ...
    async def get_settings(self, page: int=1, page_size: int=10):
        # async with self.db_session as session:
            # Calculate total number of records
            total_query = await self.db_session.execute(select(func.count(DBCameraSetting.id)))
            total_records = total_query.scalar_one()
    ...
    async def update_setting(self, setting_id: int, setting: CameraSettingUpdate):
        # async with self.db_session as session:
            ...
            try:
                db_setting = await self.db_session.merge(db_setting)
                update_data = setting.dict(exclude_unset=True)
                for key, value in update_data.items():
                    setattr(db_setting, key, value)
                await self.db_session.commit()
                await self.db_session.refresh(db_setting)
                return db_setting
            except SQLAlchemyError as error:
                await self.db_session.rollback()
    ...
    async def delete_setting(self, setting_id: int):
        # async with self.db_session as session:
            ...
            try:
                db_setting = await self.db_session.merge(db_setting)
                await self.db_session.delete(db_setting)
                await self.db_session.commit()
                return db_setting
            except SQLAlchemyError as error:
                await self.db_session.rollback()
...
class CameraOperation(CrudOperation):
    ...
    async def get_cameras(self, page: int=1, page_size: int=10):
        # async with self.db_sself.ession as session:
            # Calculate total number of records
            total_query = await self.db_session.execute(select(func.count(DBCamera.id)))
            total_records = total_query.scalar_one()
    ...
    async def delete_camera(self, camera_id: int):
        # async with self.db_session as session:
            ...
            try:
                ...
                await self.db_session.delete(db_camera)
                await self.db_session.commit()
                return db_camera
...
class LprSettingOperation(CrudOperation):
    ...
    async def get_setting_by_name(self, name: str):
        # async with self.db_session as session:
            query = await self.db_session.execute(select(DBLprSetting).where(DBLprSetting.name == name))
            setting = query.unique().scalar_one_or_none()
            return setting
    ...
    async def get_settings(self, page: int=1, page_size: int=10):
        # async with self.db_session as session:
            # Calculate total number of records
            total_query = await self.db_session.execute(select(func.count(DBLprSetting.id)))
            total_records = total_query.scalar_one()
...
class LprOperation(CrudOperation):
    ...
    async def get_lprs(self, page: int = 1, page_size: int = 10):
        # async with self.db_session as session:
            # Calculate total number of records
            total_query = await self.db_session.execute(select(func.count(DBLpr.id)))
            total_records = total_query.scalar_one()
    ...
    async def create_lpr(self, lpr):
        # async with self.db_session as session:
            try:
                ...
                result = await self.db_session.execute(select(DBLprSetting))
                default_settings = result.scalars().all()
                for setting in default_settings:
